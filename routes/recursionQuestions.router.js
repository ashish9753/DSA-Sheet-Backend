...
// Get all recursion questions with user progress
router.get('/', auth, async (req, res) => {
  try {
    const questions = await Question.find({ topic: 'Recursion' }).sort({ difficulty: 1, name: 1 });
...
router.get('/stats/summary', auth, async (req, res) => {
  try {
    const total = await Question.countDocuments({ topic: 'Recursion' });
    const allRecursionQuestions = await Question.find({ topic: 'Recursion' }, '_id');
    const recursionIds = allRecursionQuestions.map(q => q._id);

    const userProgress = await UserProgress.find({
      user: req.userId,
      question: { $in: recursionIds },
      completed: true
    });
