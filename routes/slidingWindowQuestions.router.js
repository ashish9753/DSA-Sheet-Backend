...
// Get all sliding window questions with user progress
router.get('/', auth, async (req, res) => {
  try {
    const questions = await Question.find({ topic: 'Sliding Window' }).sort({ difficulty: 1, name: 1 });
...
router.get('/stats/summary', auth, async (req, res) => {
  try {
    const total = await Question.countDocuments({ topic: 'Sliding Window' });
    const allQuestions = await Question.find({ topic: 'Sliding Window' }, '_id');
