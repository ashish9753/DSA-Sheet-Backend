...
// Get all heaps questions with user progress
router.get('/', auth, async (req, res) => {
  try {
    const questions = await Question.find({ topic: 'Heaps' }).sort({ difficulty: 1, name: 1 });
...
router.get('/stats/summary', auth, async (req, res) => {
  try {
    const total = await Question.countDocuments({ topic: 'Heaps' });
    const allQuestions = await Question.find({ topic: 'Heaps' }, '_id');
