...
// Get all greedy algorithms questions with user progress
router.get('/', auth, async (req, res) => {
  try {
    const questions = await Question.find({ topic: 'Greedy Algorithms' }).sort({ difficulty: 1, name: 1 });
...
router.get('/stats/summary', auth, async (req, res) => {
  try {
    const total = await Question.countDocuments({ topic: 'Greedy Algorithms' });
    const allQuestions = await Question.find({ topic: 'Greedy Algorithms' }, '_id');
