...
// Get all stack and queues questions with user progress
router.get('/', auth, async (req, res) => {
  try {
    const questions = await Question.find({ topic: 'Stack and Queues' }).sort({ difficulty: 1, name: 1 });
...
router.get('/stats/summary', auth, async (req, res) => {
  try {
    const total = await Question.countDocuments({ topic: 'Stack and Queues' });
    const allQuestions = await Question.find({ topic: 'Stack and Queues' }, '_id');
